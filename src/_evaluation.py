...
def _turn_solution_scores_into_choose_count(sorted_solution_scores, topk):
    ...
    wrapped = True if type(sorted_solution_scores[0][0]) == list else False  # Unwrapped for our method 
    result = []
    if wrapped:
        ...
        for score in topk_scores:
            solutions = [s[0] for s in sorted_solution_scores if s[1] == score] # solutions with `score`
            result.append((solutions, 1)) # append a tuple of (solutions, choose_count)
...
def hypergeometric_distribution(n, c, k, t):  
    if t > c or t > k or k-t > n-c:  
        return 0  
    return comb(c, t) * comb(n - c, k - t) / comb(n, k)  


def _estimate_precision_at_k(
    num_samples: Union[List[int], np.ndarray],
    num_correct: Union[List[int], np.ndarray],
    k: int
) -> np.ndarray:
    """
    Estimates precision@k of each problem and returns them in an array.
    """
    
    return np.array([sum([t / k * hypergeometric_distribution(int(n), int(c), k, t) for t in range(0, k+1)]) for n, c in zip(num_samples, num_correct)])
